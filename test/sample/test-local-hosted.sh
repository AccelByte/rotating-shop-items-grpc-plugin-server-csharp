...
(cd src/AccelByte.PluginArch.ItemRotation.Demo.Server && dotnet run) & GRPC_SERVER_PID=$!
...
(cd src/AccelByte.PluginArch.ItemRotation.Demo.Client && dotnet run -- --grpc-target ${NGROK_URL#*://})